...
public:
    ...
    {
        HttpPost(HttpClient &client, std::string_view host, std::string_view port, std::string_view target, int version, std::string_view content)
            : _client(client), _host(host), _port(port), _target(target), _version(version), _content(content)
        {
        }
     ...
     * @return HttpPost 
     */
    virtual HttpPost co_post(std::string_view url, std::string_view content) = 0;
};
